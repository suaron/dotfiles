...
function htmlLogger(string) {
	if (typeof(string) === 'undefined') string = 'No input';
	...
	div.id = "result";
	div.innerHTML = "<strong>" + string + "</strong>";
...
(function(){
	...
	// Test for time strings and append time format (only) if needed
	// the second argument is a set of options for handling
		// separator: (String) will separate the time from the date with the given string
		// timeFormat: (String) override the default time format
	// "format string".addTime(originalString, {separator: ', '})
	String.prototype.addTime = function(input, options) {
		var defaults = {
			separator: ' ',
		...
		if ((/(\d{1,2}:\d{1,2}|\d{1,2}\s*[ap]m?\b|\d+[mh]|min(utes?)?|hours?)/i).test(input)) {
			return this + defaults.separator + defaults.timeFormat;
		}
		return this;
	};

	function handleInputString(input) {
		input = input.replace(/^>>(.*?)</,"$1");

		var re = new RegExp("^(i(?:so)?|s(?:hort)?|l(?:ong)?|m(?:ed(?:ium)?)?|f(?:ull)?|d|x|u|t|n)?(r(\\d*))?(-)? (.+)", 'i'),
			options = {
				format: dateFormat,
				future: true,
				round: false
		...
		if (re.test(input)) {
			...
			if (!res.length === 6) {
				throw new UserException("Invalid string format");
			}

			input = res[5];

			if (res[1]) {
				formatStr = res[1].substr(0,1);

				switch (formatStr) {
				case 's': // short: 11/12/2016
					fmt = '{short}'.addTime(input);
					break;
				case 'm': // medium: November 12, 2016
					fmt = '{medium}';
					break;
				case 'l': // long: November 12, 2016 12:00 AM
					fmt = '{medium}'.addTime(input, {separator: ', '});
					break;
				case 'f': // full: Saturday, November 12, 2016 12:00 AM
					fmt = '{full}';
					break;
				case 'n': // nice (ordinal date, "at" for time): Saturday, November 12th at 3:00 PM
					fmt = '%A, %B {do}'.addTime(input, {separator: ' at ', timeFormat: '{time}'});
					break;
				case 'N': // nice with year: Sunday, November 13th, 2016 at 3:00 PM
					fmt = '%A, %B {do}, %Y'.addTime(input, {separator: ' at ', timeFormat: '{time}'});
					break;
				case 'd': // hyphenated date only: 2016-11-13
					fmt = '%Y-%m-%d';
					break;
				case 'i': // ISO timestamp: Sat Nov 12 2016 3:00 PM
					fmt = '{stamp}';
					break;
				case 't': // time (12hr): 06:30 PM
					fmt = '%I:%M %p';
					break;
				case 'T': // time (24hr): 16:30
					fmt = '%H:%M';
					break;
				case 'u': // unix timestamp:
					fmt = '{X}'
					break;
				case 'U': // unix timestamp (milliseconds): 1479070800
					fmt = '{x}'
					break;
				case 'x': // localized date/time string: Sat Nov 12 2016 1:20 PM
					fmt = '%c';
					break;
				default:
					fmt = dateFormat.addTime(input);
					break;
				}
			...
			options.format = dateFormat.addTime(input);
		}

		var tomRE = /\bto[md]\b/i,
			abbrRE = /^(\+|\-)?(\d+)([hmdy]\b|\s(?:days?|months?|years?|min(?:utes)?)|$)/i;

		if (tomRE.test(input)) {
			input = input.replace(/\btod\b/i, 'today').replace(/\btom\b/i, 'tomorrow');
		}


		if (abbrRE.test(input)) {
			res = abbrRE.exec(input);

			var past = false,
				inc = 'm';

			if (/\bago\b/.test(input)) {
				input = input.replace(/\bago\b/,'').trim();
				options.future = false;
				past = true;
			}
			...
			if (res[2]) {
				if (res[3]) {
					if (/mo(n(ths?)?)?/.test(res[3].trim()))
						inc = 'mon';
					else
						inc = res[3].trim().toLowerCase().substr(0,1);
				}

				switch (inc) {
					case 'mon':
						input = expandRelative(res[2] + ' months', past);
						break;
					case 'm':
						input = expandRelative(res[2] + ' minutes', past);
						break;
					case 'd':
						input = expandRelative(res[2] + ' days', past);
						break;
					case 'y':
						input = expandRelative(res[2] + ' years', past);
						break;
					case 'h':
						input = expandRelative(res[2] + ' hours', past);
						break;
					default:
						input = expandRelative(res[2] + ' minutes', past);
						break;
	...
	function expandRelative(str, past) {
		if (past)
			return str + ' ago';
		else
			return 'in ' + str;
	...
	function convertDate(string) {
		var obj;
		try {
			obj = handleInputString(string);
		} catch(e) {
			return string + '(' + e.message + ')';
		}

		if (!Sugar.Date.isValid(Sugar.Date(obj.string))) {
			return string;
		}

		var input = obj.string,
			options = obj.options,
			strISO = new Sugar.Date(input),
			strISOminutes = parseInt(Sugar.Date.format(strISO, '%M'), 10),
			output,
			weekdayRE = /(\bmon|\btue|\bwed|\bthu|\bfri|\bsat|\bsun|\bdays)(?!.*(\d+|ago))/i;

		if (options.future && Sugar.Date.isPast(strISO) && weekdayRE.test(input) && !(/last/i).test(input)) {
			strISO = Sugar.Date.advance(strISO, {weeks: 1});
		}
		if (options.round && (strISOminutes % options.round !== 0)) {
			var hours = 0,
				minutes = Math.round(strISOminutes / options.round) * options.round;
			while (minutes > 60) {
				hours += 1;
				minutes -= 60;
			}
			Sugar.Date.addHours(strISO, hours);
			Sugar.Date.set(strISO, {minutes: minutes});
		}
		output = Sugar.Date.format(strISO, options.format);
	 	return output;
	};

	var kme = Application('Keyboard Maestro Engine'),
		input = kme.getvariable('input');

	dateFormat = kme.getvariable('dateFormat'),
	timeFormat = kme.getvariable('timeFormat');
	dateFormat = (typeof dateFormat === 'undefined' || dateFormat === '') ? '%Y-%m-%d' : dateFormat;
	timeFormat = (typeof timeFormat === 'undefined' || timeFormat === '') ? '{h}:{mm}%P' : timeFormat;

	return  convertDate(input);

//	var input = "f today";
//
//	htmlLogger(convertDate(input, '%Y-%m-%d %I:{mm}%p'));
})();
